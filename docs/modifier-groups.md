...
A modifier group is **a question your menu asks about a dish**, together with the answers a customer can pick from. "Choose your side" with *fries / salad / rice*. "Sugar level" with *normal / less / none*. "Add extras" with *extra cheese / fried egg / no onions*.
...
- **Selection** — whether the customer picks **one** answer or **several**.
- **Answers** — how many choices this question offers.
...
- **Selection** — **Single** shows radio buttons, so exactly one answer can be chosen. **Multiple** shows checkboxes.
- **Minimum Answers** — set **1** to make the question compulsory: the customer cannot add the dish to the cart without answering. Set **0** to let them skip it.
- **Maximum Answers** — the cap on a multiple-choice question, e.g. *pick any 3 toppings*. Leave it empty for no limit. It is ignored when the selection is Single, which always allows exactly one.
...
One row per choice. Add them with **Add Answer**.

- **Answer** — what the customer picks, e.g. *Extra cheese*. Required, and translatable.
- **Price Change** — **leave this at 0.00 for now.** See the warning below.
- **Pre-selected** — this answer is ticked by default. On a single-selection group only the first pre-selected answer is used.
- **Status** — hide one answer without deleting it or touching the rest of the group.
...
- **Required** — leave on **Inherit from group** to use the group's own Minimum Answers. Choose **Required** or **Optional** only where this one dish should differ, e.g. a side is compulsory with the set meal but optional à la carte.
...
2. **Modifier group "Choose your protein"** — Selection: Single, Minimum Answers: **1** (compulsory). Answers: *Ayam Goreng*, *Rendang*, *Sambal Sotong*.
3. **Modifier group "Add extras"** — Selection: Multiple, Minimum: 0, Maximum: 3. Answers: *Extra sambal*, *Fried egg*, *Extra peanuts*.
...
- **Deleting a group** removes it from every dish that asked it. Its answers go with it, and come back with it if it is restored. If you only want it off the menu for now, set it **Inactive** instead.
