@php
    $hasRail = $showFilters && $sidebarPos !== 'none' && !empty($collections);
@endphp
...
<section class="saffron-section saffron-listing">
    <div class="saffron-container">
        <div class="row g-4 g-lg-5">
            ...
            @if($hasRail && $sidebarPos === 'left')
                @include('partials.sections.commerce.CollectionGrid.rail')
            @endif

            <div class="{{ $hasRail ? 'col-lg-9' : 'col-12' }}">
                ...
                @if($dishes->isEmpty())
                    <div class="saffron-menu__empty">
                        <p class="mb-0">{{ __('Nothing on this part of the menu just yet.') }}</p>
                    </div>
                @else
